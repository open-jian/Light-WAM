...
class LightWAM(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.history_enabled:
            ...
            if self.history_raw_stride != int(self.vae.temporal_downsample_factor):
                raise ValueError(
                    "History raw stride must match the VAE temporal downsample factor, "
                    f"got {self.history_raw_stride} and {self.vae.temporal_downsample_factor}."
                )
    ...
    def _sample_history_window_size(self) -> int:
        """Sample one shared window size for the whole distributed batch."""
        if not self.history_enabled:
            return 0
        if not self.training:
            return self.history_max_size
        ...
        if not dist.is_available() or not dist.is_initialized() or dist.get_rank() == 0:
            sampled_size.random_(self.history_min_size, self.history_max_size + 1)
    ...
    def _prepare_history_latents(
        ...
        window_size = self._sample_history_window_size()
        # Fixed max-size candidates are oldest -> newest.  Slice before VAE
        # encoding so raw-image training only encodes the sampled recent window.
        history_valid_mask = history_valid_mask[:, -window_size:].contiguous()
        ...
        if cached_history is not None:
            ...
            history_latents = cached_history[:, :, -window_size:].to(
                device=self.device,
                dtype=self.torch_dtype,
                non_blocking=True,
            ).contiguous()
            ...
            raw_history = raw_history[:, :, -window_size:].contiguous()
    ...
    def _prepare_inference_history_latents(
        ...
        if not self.history_enabled:
            if history_images is not None and int(history_images.numel()) > 0:
                raise ValueError("`history_images` was provided but history memory is disabled.")
            return None, None
    ...
    def build_inputs(self, sample, tiled: bool = False):
        ...
        history_latents, history_valid_mask = self._prepare_history_latents(
            sample=sample,
